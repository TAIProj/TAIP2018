...
while k < 10:
    #GEORGIA (STATE)
    r = requests.get('http://localhost:8080/waze/traffic-notifications?latBottom=47.154723&latTop=47.155686&lonLeft=27.601529&lonRight=27.603557')
    ...
    while i < 500:
        try:
            ...
            if r2.json()['alerts'][i]['type'] != 'POLICE':
                ...
                print (r.json()['alerts'][i]['latitude'])
                print (r.json()['alerts'][i]['lo(ngitude'])
                print (r.json()['alerts'][i]['subType'])
                print (r.json()['alerts'][i]['numOfThumbsUp'])
